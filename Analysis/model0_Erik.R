...
require(deSolve)
require(Rcpp)
sourceCpp( 'Analysis/model0_Erik.cpp' )
...
.mweight <- function( rowdeme, coldeme ){
  ...
  colrisk <- as.numeric( regmatches( coldeme, regexec( "riskLevel([0-9])", rowdeme) )[[1]][2] )
...
#prRecipMat[m,] <- 0
prRecipMat[which(is.na(prRecipMat))] <- 0
prRecipMat[m,m] <- 1
...
View(tfgy[[2]][312][[1]]) #1984
View(tfgy[[2]][313][[1]]) #1985 when diagnosis starts
...
lapply(tfgy[[3]], function(x) print(x[34]))

GGteste <- tfgy[[3]][313][[1]][which(grepl(pattern = "care1", x = rownames(tfgy[[3]][573][[1]]))),
                    which(grepl(pattern = "care2", x = colnames(tfgy[[3]][573][[1]])))]

GG_care1_to_care2 <- lapply(tfgy[[3]], function(x) x[which(grepl(pattern = "care1", x = rownames(x))),
...
plot(times_year,unlist(lapply(teste_birth, function(x) sum(x))))
