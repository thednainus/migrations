...
theta <- c( age_assort_factor = .5
            ...
            , inc_scale = 1.5 # based on docking (see below) #.03

            , max_diag_rate = 1.5 # based on docking (see below)  #1/3 # time 2 diag of 3 yrs
            , diag_rate_85 = 0
            , accel_diag_rate = 1.5 # based on docking (see below ) #1 / 7 # accel of logistic function
...
X11(); plot( times_year, tr )
X11(); plot( times_year, di )

o <- ode(y=y0, times=times_day, func=dydt, parms=list()  , method = 'adams')
tfgy <- .tfgy( o )

teste_birth <- lapply(tfgy[[2]], function(x) x[which(grepl(pattern = "care2", x = colnames(x)))])
