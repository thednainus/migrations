...
#this version of the script I will increase time resolution
#to weeks to see if improves the fit
#I will use the same parameters values as in fit3 and call it fit3_weeks
...
filenamerds <- as.character(get_params[2])

parameters <- readRDS(system.file("data/params3dim_v3.RDS", package = "SDBenefits"))

##
#load parameters from LHS

parameters <- readRDS(filenamerds)
parameters <- parameters[line_number,]
...
theta <- c(
  ...
  inc_scale = parameters$inc_scale, #  incidence scale (inc_scale): based on docking (see below) # initial = .03
  max_diag_rate = parameters$max_diag_rate  , #  based on docking (see below) # initial = 1/3 (time 2 diag of 3 yrs)
  #diag_rate_85 =1/10,
  diag_rate_85 = 0,
  #diag_rate_85 = parameters$diag_rate_85,
  accel_diag_rate = parameters$accel_diag_rate , #  based on docking (see below ) # initial = 1/7 # accel of logistic function
...
#aggregate data by year to fit data to new diagnosis in San Diego
