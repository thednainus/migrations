#get the HOD (hihest posterior density)
library(coda)
...
#run1
mcmc_results1 <- list.files("Analysis_SD/phydynR/mcmc_results/ICL_cluster/importation_rate/low/region1000global100/all_tree/run1/results", recursive = TRUE, full.names = TRUE)
#run2
mcmc_results2 <- list.files("Analysis_SD/phydynR/mcmc_results/ICL_cluster/importation_rate/low/region1000global100/all_tree/run2/results", recursive = TRUE, full.names = TRUE)

#run1
mcmc_results_iter.data1 <- mcmc_results1[grepl("iter.rdata", mcmc_results1)]
mcmc_results_iter.data1 <- mcmc_results_iter.data1[-c(5,1,16,35,48,3,38,4,12)]
length(mcmc_results_iter.data1)
mcmc_results_out1 <- mcmc_results1[grepl("out_sim.RDS", mcmc_results1)]
mcmc_results_out1 <- mcmc_results_out1[-c(5,1,16,35,48,3,38,4,12)]
length(mcmc_results_out1)

#run2
mcmc_results_iter.data2 <- mcmc_results2[grepl("iter.rdata", mcmc_results2)]
mcmc_results_iter.data2 <- mcmc_results_iter.data2[-c(5,1,16,35,48,3,38,4,12)]
length(mcmc_results_iter.data2)

mcmc_results_out2 <- mcmc_results2[grepl("out_sim.RDS", mcmc_results2)]
mcmc_results_out2 <- mcmc_results_out2[-c(5,1,16,35,48,3,38,4,12)]
...
for(j in 1:length(mcmc_results_iter.data1)){
  ...
  if(i == 801){
    ...
    HPD_all <- rbind(HPD_all, quantiles_results)
  }
}
