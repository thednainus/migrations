...
source("load_data.R")
#source("Analysis_SD/phydynR/model1.1_phydynR.R")
source("model1.1_phydynR.R")
...
EUPPER <-  c(1e5, 500,   1/5, 1/1., rep(2, length(betaNames) ) )
...
priordensity <- function(theta){
  ...
  o = dlnorm(theta['src1980'], log(2e4), 1/4 , log = TRUE ) +
      ...
      dlnorm(theta['srcMigrationRate'], log(1/20), 1/4 , log = TRUE ) +
...
sampler <- function(n = 1){
  ...
  I1980 <- rexp (n, 1/10 )
  srcMigrationRate <- rlnorm(n , log( 1/20 ), 1/4 )
