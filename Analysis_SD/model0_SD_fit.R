...
##
#load parameters from LHS


parameters <- readRDS(system.file("data/params4dim_v15.RDS", package = "SDBenefits"))
parameters <- readRDS("older_params/params4dim_v")
...
for(param_i in 1:nrow(parameters)){
  ...
  theta <- c(
    ...
    inc_scale = parameters$inc_scale[param_i]  , #  incidence scale (inc_scale): based on docking (see below) # initial = .03
    max_diag_rate = parameters$max_diag_rate[param_i]  , #  based on docking (see below) # initial = 1/3 (time 2 diag of 3 yrs)
    #diag_rate_85 =1/10,
    diag_rate_85 = parameters$diag_rate_85[param_i],
    accel_diag_rate = parameters$accel_diag_rate[param_i] , #  based on docking (see below ) # initial = 1/7 # accel of logistic function
  ...
  #y0[ CARE_COORDS$care1 ] <- (1 / length( CARE_COORDS$care1 )) + parameters$y0_init[param_i]
  ...
  #phil_inc <- data.matrix( read.table( 'Analysis_SD/incidence.tsv') )[,1]
  phil_inc_times <- incidence$year
  ...
  new_diagnosis <- care2_total
  new_diagnosis["only_year"] <- unlist(lapply(new_diagnosis$year, function(x) strsplit(as.character(x), split = ".", fixed = TRUE)[[1]][1]))
  #new_diagnosis$only_year <- as.factor(new_diagnosis$only_year)


  newDx_pop1_agg <- aggregate(diagnosed ~ only_year ,
                              data = new_diagnosis,
                              FUN=sum)
  newDx_pop1_agg$diagnosed <- round(newDx_pop1_agg$diagnosed)

  #change name so I can use the same function in package HIVepisim
  names(newDx_pop1_agg)[2] <- "newDx_pop1"

  #cheate filename to save data
  filename <- paste("param", param_i,
                    "inc_scale", parameters$inc_scale[param_i],
                    "max_diag_rate", parameters$max_diag_rate[param_i],
                    "accel_diag_rate", parameters$accel_diag_rate[param_i],
                    sep = "_")
  ...
  log_weights_dx_df <- data.frame(inc_scale = parameters$inc_scale[param_i],
                                  max_diag_rate = parameters$max_diag_rate[param_i],
                                  accel_diag_rate = parameters$accel_diag_rate[param_i],
...
saveRDS(log_weights_dx_df_all, "log_weights_dx_df.RDS")
...
w =   exp( log_weights_dx_df_all[,4]  - max( log_weights_dx_df_all[,4] ) )
